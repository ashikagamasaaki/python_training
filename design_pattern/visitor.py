...
・データ構造を表すクラス、それに対する操作を行うクラスを分離
・「操作を行うクラス」が「データ構造を表すクラス」を訪問し、順に処理
・「データ構造を表すクラス」には操作について記述せず、訪問者を受け入れるメソッドを用意
...
Group1 -> Group2 -> Employee1
1.ClientがListVisitorをインスタンス化
2.最上位であるGroup1のacceptメソッドにListvisitorを渡す
3.Group1がVisitメソッドを呼び出し、自身の情報を渡す
4.ListVisitorがGroup1の子要素であるGroup2に訪問する
5.Group2がVisitメソッドを呼び出し、自身の情報を渡す
6.ListVisitorがGroup2の子要素であるEmployee1に訪問する
7.Employee1がvisitメソッドを呼び出し、自身の情報を渡す
8.Employee1には子要素がないので結果がClientに返される
"""
